...
export default function HomeBanner() {
  // Properly reset initial banner values when navigating to home from home
  ...
  const logoStylesZIndex = useTransform(progress, [0, 0.8, 1], [20, 102, 102]);
  const bannerPaddingLeft = useTransform(progress, animationSteps, [192, 0]);
  const bannerPaddingRight = useTransform(progress, animationSteps, [192, 0]);
  // const bannerMarginBottom = useTransform(progress, animationSteps, [
  //   "inherit",
  //   0,
  // ]);
  ...
  useEffect(() => {
    getWindowWidth();
    window.addEventListener("resize", getWindowWidth); // FIXME: doesn't work very well
  ...
  useEffect(() => {
    const imageWidth = bannerImageRef.current.children[0].offsetWidth;
    // const imageHeight = Math.round(imageWidth * -0.17 + 206); // FIXME: adjust for real image dimensions
  ...
  return (
    <LayoutGroup>
      <motion.div
        ref={bannerImageRef}
        layout
        className={`self-center w-full pt-32 z-0 fixed`} // mt-32
        style={{
      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.div
            ref={scrollRef}
            className="fixed left-1/2 transform -translate-x-1/2" // absolute bottom-12 z-20
            style={{
              top: logoContainerTop,
              zIndex: logoStylesZIndex,
            }}
            exit={{ opacity: 0 }}
          >
            <motion.svg
              ...
              viewBox="0 0 192 45"
              className="fill-primary-light m-auto" // z-20
            >
